...
export function renderTopCasino() {
  ...
  const h2 = createEl("h2", {
    ...
    text: `I Migliori Casinò ${country}`,
  });
  ...
  function renderCards(visibleCount) {
    ...
    appState.website.offers.slice(0, visibleCount).forEach((offer) => {
      ...
      const h4 = createEl("p", { className: "welcome", text: "Bonus di benvenuto" });
      const p = createEl("p", {
        className: "offer",
        text:
      ...
      btn.textContent = "Richiedi il bonus";
  ...
  if (appState.website.offers.length > 8) {
    ...
    allBtn.textContent = "Tutto il casinò";
