...
export function renderBonusDetails() {
  ...
  const renderRow = (offer) => {
    ...
    img.alt = `${offer.name} in ${SITE_NAME}`;
    img.title = `${offer.name} in ${SITE_NAME}`;
    img.width = 140;
    img.height = 56;
    img.className = "img";
    img.loading = "lazy";
    ...
    cellBonus.innerHTML = "<p>Willkommensbonus</p>";
    ...
    get.textContent = "Bonus Erhalten";
  ...
  function createAccordion(offer) {
    ...
    r1.appendChild(createEl("p", { text: "Maximaler Betrag:" }));
    r1.appendChild(
      createEl("p", {
    ...
    lastBtn.textContent = "Bonus Erhalten";
